...
export async function createResetCode() {
  ...
  await setSetting('reset_salt', salt);
  await setSetting('reset_code_hash', sha256(salt + ':' + code));
  await setSetting('reset_expires', String(Date.now() + RESET_TTL_MS));
  return code;
}

export async function verifyResetCode(code) {
  const hash = await getSetting('reset_code_hash');
  const salt = await getSetting('reset_salt');
  const exp = Number(await getSetting('reset_expires') || 0);
  if (!hash || !salt) return false;
  if (Date.now() > exp) return false;
  return timingEqual(sha256(salt + ':' + String(code)), hash);
}

export async function clearResetCode() {
  await delSetting('reset_code_hash');
  await delSetting('reset_salt');
  await delSetting('reset_expires');
}
